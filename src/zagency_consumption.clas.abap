CLASS zagency_consumption DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zagency_consumption IMPLEMENTATION.


 METHOD if_rap_query_provider~select.
DATA:
  lt_business_data TYPE TABLE OF zmst1009_agency=>tys_z_travel_agency_es_5_type,
     ...
     lo_client_proxy = /iwbep/cl_cp_factory_remote=>create_v2_remote_proxy(
       EXPORTING
          is_proxy_model_key       = VALUE #( repository_id       = 'DEFAULT'
                                              proxy_model_id      = 'ZMST1009_AGENCY'
...
ENDIF.
*lo_request->set_top( CONV i( top ) )->set_skip( CONV i( skip ) ).
